...
//        }
        ...
        if($store){
            foreach ($users as $user){
                $user_id= User::where('device_token', $user)->first();
                $notify = new Notification();
                $notify->type = $type;
                $notify->user_id = $user_id->getId();
                $notify->title = $title;
                $notify->message = $msg;
...
//        }
    ...
    public static function Payout($iban,$swift_code,$name,$amount,$address_1,$address_2,$address_3,$request_refund_id): array
    {
        $email = 'faisal-hmood@outlook.com';
        $password = 'passion2020$';
        $ch = curl_init();
